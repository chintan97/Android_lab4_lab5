...
public class StudentDataQueries {
    ...
    private String[] studentColumns = {
            StudentReaderDbHelper.StudentEntry._ID,
    ...
    private String[] courseColumns = {
            StudentReaderDbHelper.CourseEntry._ID,
    ...
    public StudentModel createStudent(StudentModel studentModel){
        ...
        long insertId = database.insert(StudentReaderDbHelper.StudentEntry.TABLE_NAME, null, values);
        Log.i(getClass().getName(), String.valueOf(insertId));
        Cursor cursor = database.query(StudentReaderDbHelper.StudentEntry.TABLE_NAME,
                studentColumns, StudentReaderDbHelper.StudentEntry._ID + " = " + insertId,
                null, null, null, null);
        cursor.moveToFirst();
    ...
    public boolean updateStudent(StudentModel studentModel){
        ...
        return database.update(StudentReaderDbHelper.StudentEntry.TABLE_NAME, cv,
                StudentReaderDbHelper.StudentEntry._ID + " = " + studentModel.getId(),
                null) > 0;
    }
    ...
    private StudentModel getStudentDataFromCursor(Cursor cursor){
        return new StudentModel(cursor.getLong(0), cursor.getString(1),
                cursor.getString(2), cursor.getInt(3));

    }

    public boolean isAppRunningFirstTime(){
        boolean isFirst = mContext.getSharedPreferences(SHARED_PREF_STUDENT, Context.MODE_PRIVATE).getBoolean(IS_FIRST_TIME, true);
        if (isFirst){
